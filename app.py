...
def send_sms(phone, message):
    ...
    try:
        ...
        print("TextBee response:", response.status_code, response.text)
        return response.ok
...
@app.route("/")
def home():
    products = Product.query.filter_by(active=True).order_by(Product.id.asc()).all()
...
def verify_otp():
    if request.method == "POST":
        ...
        customer = Customer.query.filter_by(mobile=user_mobile).first()
        if customer:
        ...
        session["user_address"] = customer.address or ""
...
def account():
    ...
    total_orders = Order.query.filter_by(mobile=user_mobile).count()
    recent_orders = (
        Order.query.filter_by(mobile=user_mobile)
...
def place_order():
    ...
    try:
        ...
        if not address:
            return jsonify(
                {"success": False, "message": "Please enter your delivery address."}
            ), 400
        ...
        for item in items:
            ...
            if not product:
                return jsonify(
                    {"success": False, "message": f"Product '{product_name}' is no longer available."}
                ), 400

            if not product.active:
                return jsonify(
                    {"success": False, "message": f"{product.name} is currently unavailable."}
                ), 400

            if quantity <= 0:
                return jsonify(
                    {"success": False, "message": f"Invalid quantity for {product.name}."}
                ), 400
        ...
        calculated_total = sum(
            product.price * quantity for product, quantity in products_by_item
        )
        ...
        sms_message = (
            f"PICKELS STORE: Order #{order.id} placed successfully. "
            f"Total ₹{calculated_total:.2f}. Payment: {payment_method.upper()}. "
            f"Status: Pending."
        )
        send_sms("+91" + mobile, sms_message)
...
def cancel_order(order_id):
    ...
    send_sms(
        "+91" + mobile,
        f"PICKELS STORE: Order #{order.id} has been cancelled successfully."
    )
...
def order_history():
    ...
    orders = Order.query.filter_by(mobile=mobile).order_by(Order.id.desc()).all()

    for order in orders:
        order.created_at_ist = (
            order.created_at + timedelta(hours=5, minutes=30)
            if order.created_at
            else None
        )
        ...
        if isinstance(raw_items, list):
            for item in raw_items:
                if isinstance(item, dict):
                    ...
                    display_items.append({"name": item, "price": 0, "quantity": 1})
...
def track_order():
    ...
    if request.method == "POST":
        ...
        mobile = request.form.get("mobile", "").strip().replace(" ", "")
        ...
        order = Order.query.filter_by(id=order_id, mobile=mobile).first()
        if not order:
            return render_template(
                "track_order.html",
                order=None,
...
def admin():
    ...
    for order in orders:
        try:
            ...
            for raw_item in raw_items:
                ...
                product = Product.query.filter(
                    db.func.lower(Product.name) == item_name
                ).first()
                if product:
    ...
    total_sales = sum(
        order.total for order in orders if order.status != "Cancelled"
    )
...
def update_order(order_id):
    ...
    message = f"PICKELS STORE: Order #{order.id} status updated to {order.status}."
    send_sms("+91" + order.mobile, message)
...
def admin_customers():
    ...
    for customer in customers:
        order_count = Order.query.filter_by(mobile=customer.mobile).count()
    ...
    return render_template("admin_customers.html", customers=customer_data)
...
def admin_customer_details(customer_id):
    ...
    orders = Order.query.filter_by(mobile=customer.mobile).order_by(Order.id.desc()).all()
